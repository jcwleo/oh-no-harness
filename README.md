...
Claude Code autocomplete shows each skill's expected arguments through `argument-hint`, for example `clarify [--deep]` and `planning [--ral] [spec-or-request]`. Each skill also declares positional `arguments` for Claude Code substitution, then treats `$ARGUMENTS` as the authoritative raw input so optional flags do not mis-map positional values. `when_to_use` sharpens automatic routing without adding hidden runtime logic.
