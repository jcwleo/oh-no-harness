...
Codex exposes skills through native skill loading. Codex `spawn_agent` availability is host-policy controlled: some Codex hosts permit natural
dispatch when the active skill, execution mode, risk, and scope justify it, while
other hosts permit `spawn_agent` only after the user explicitly asks for
subagents, delegation, or parallel agent work. Treat the host tool definition as
authoritative. Do not treat ordinary skill invocation by itself as a Codex
subagent trigger; there must be a concrete role, isolated scope, expected
output, integration plan, and host authorization.
...
- Codex: use `spawn_agent` only when the current host tool definition allows it,
  the active skill's documented dispatch policy allows the role, and the work has
  an isolated read-only scope, disjoint write ownership, or an independent
  review/verification responsibility. Explicit phrases such as `subagent`,
  `spawn`, `delegate`, `parallel agents`, `parallel subagents`, or `one agent
  per` are sufficient dispatch signals. Natural dispatch is allowed only on hosts
  whose tool definition permits it. For Ralph execution, the
  ...
  `agents/<role>.md` prompt content in the spawned-agent message. When host
  policy permits dispatch and an eligible batch exists, spawn all independent non-blocking agents in the batch before waiting for results. When no concrete
  dispatch-worthy scope exists or the host policy does not authorize dispatch,
  perform the role inline, preserve the same role boundaries, and record the
  inline fallback reason when the active skill requires it.
