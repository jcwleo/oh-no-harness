---
name: code-reviewer
description: Use proactively inside active Oh No Harness workflows to review changed code or prompts; the caller owns approval and handoff gates.
...
<!-- Source: plugins/oh-no-harness/docs/agent-core/code-reviewer.md -->
...
the lenses across hosts. The caller synthesizes both reviews' findings,
deduplicated and tagged with host provenance. When the opposite host is
unavailable in default mode, the calling skill runs the Same-Host Parallel
Fallback (two same-host reviewers with distinct assigned perspectives,
synthesized) per its own review contract; require-cross-host mode blocks.
