...
plugins/oh-no-harness/skills-claude/<name>/SKILL.md # Generated Claude Code-facing runtime skill document (13 total: 11 cross-platform + 2 Claude-only setup skills)
...
- Public skill surface is the 13 skills listed in `AGENTS.md` — 11 cross-platform workflow skills plus 2 Claude-Code-only, human-invoke-only setup skills (`install-statusline`, `configure-subagents`) — for 13 Claude-visible total; the Codex surface is the 11 workflow skills only. Do not add user-invocable skills without updating the manifest's `skills` array and the validator's `PUBLIC_SKILLS` list.
- Claude Code command wrappers must mirror those same 13 names only (12 commands before `configure-subagents`, 13 after). Keep them thin: argument-hint metadata, `$ARGUMENTS`, and a direct read of the matching `skills-claude/<name>/SKILL.md` file. The `install-statusline` and `configure-subagents` wrappers are the two exceptions that set `disable-model-invocation: true` (they must never be model-invoked); all others set `false`.
