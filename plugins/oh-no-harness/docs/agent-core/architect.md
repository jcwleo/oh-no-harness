...
- Check acceptance criteria coverage: whether the architecture proves the user's or
  maintainer's success signal instead of optimizing for an internal shortcut or
  convenient test.
- Check validation coverage using `docs/shared/validation-check.md` when
  measurable evidence influenced the plan. Flag task-specific, fixture-specific, or
  architecture justified only by metric movement.
