...
| HG-ralplan-handoff | ralplan | plan presented | all | combined approval/choice with approve-and-run options | prevents unapproved execution and automatic invocation | ultrawork owns automatic approval | merge if handoff owner replaces it | unique |
...
| HG-ralph-persistence | ralph | completion claim | STANDARD, THOROUGH | ledger audit | prevents missing completion evidence | LIGHT / STANDARD small-carveout not-required records | merge into VBC when session ownership converges | unique |
