...
| `opencode/preferences.js` | handwritten parser, exact-role/model validator, and safe reader for separate `opencode-subagent-models.conf` assignments; unconfigured roles receive no model field and inherit the primary |
...
| `package.json` | public `oh-no-harness` npm package metadata; exports `opencode/index.js`, includes only the OpenCode runtime and package documentation, and stays version-locked with both marketplace manifests |
...
| repository-root `scripts/test-opencode-plugin.sh` | isolated deterministic OpenCode 1.18.11 source-loading test for real host discovery, exact inventories, the native global permission ceiling, primary/role restrictive ceilings, role hard denies, finite task topology, arbitrary restriction preservation, model preferences, restart consumption, transaction safety, and project immutability; no provider credential or model call |
