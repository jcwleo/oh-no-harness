...
| `executor-codex` | `ralph`, `ultrawork`, `systematic-debugging` executor dispatch when the `codexExecutor` toggle is on (Claude Code only; ships a Claude wrapper `agents/executor-codex.md` and is NOT registered as a Codex custom agent, so on Codex the native `executor` runs directly) | returns Codex's result, the git-derived changed-file set, and the pre/post protected-set snapshots to the caller |
