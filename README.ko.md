...
**Oh No Harness**는 **Claude Code**와 **Codex**를 위한 그 workflow입니다: **10개의 stage skill**과 **11개의 role agent**가 모호한 요청을 `interview`에서 `ralplan`, 검증된 `ralph` 실행까지 끌고 가며, npm, tmux, MCP, terminal-only control plane 없이 동작합니다.
